...
export default function ServiceInterAdmin() {
    ...
    useEffect(() => {
        const fetchExchanges = async () => {
            try {
                const res = await fetch("/api/admin/serviceInterList");
                const data = await res.json();
                setExchanges(data.exchanges || []);
            } catch (err) {
                console.error("거래소 소개 목록 불러오기 오류:", err);
            }
        };
        fetchExchanges();
    }, []);
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await fetch("/api/admin/serviceInterDelete", {
                method: "POST",
    ...
    const handleRegister = () => {
        router.push("/admin/ServiceInterNew");
    };
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-gray-700 border-t border-gray-200">
                        ...
                        <tr className="text-gray-500 border-b bg-gray-50">
                            ...
                            <th className="py-3 text-left px-2">이미지</th>
                            <th className="py-3 text-left px-2">제목</th>
                        </tr>
                        </thead>
                        ...
                        {exchanges.length === 0 ? (
                            ...
                            exchanges.map((ex) => (
                                ...
                                >
                                    ...
                                    {/* ✅ 제목 클릭 시에만 상세 페이지 이동 */}
                                    <td
                                        className="text-left font-medium text-gray-800 hover:text-orange-600 transition cursor-pointer"
                                        onClick={() =>
                                            router.push(`../serviceInter/${ex.id}`)
                                        }
                                    >
                                        <div>{ex.name}</div>
                                        {ex.tag && (
